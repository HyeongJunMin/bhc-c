...
| Agent C (shared/physics/docs) | QA-PLAY-001C | done | 2026-03-03 00:00 | 한글키/카메라-샷 방향 계약 QA 스크립트 반영 및 타입체크 완료 |
...
| SYS-CORE-001A | Agent C | done | 2026-03-03 00:00 | `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | SYS-CORE-001B | `stepRoomPhysics`/충돌 루프를 `packages/physics-core/src/room-physics-step.ts`로 추출 완료 |
| SYS-CORE-001B | Agent C | done | 2026-03-03 00:00 | `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | SYS-CORE-001C | 물리 고정 상수 모듈(`room-physics-config.ts`) 분리 및 룸별 설정 스냅샷 로그 추가 완료 |
| SYS-CORE-001C | Agent C | done | 2026-03-03 00:00 | `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | PHYS-SAFE-001A | 샷 진단 로그(`shotId/tickCount/maxObservedSpeed/nanGuard/endReason`) 표준화 완료 |
| PHYS-SAFE-001A | Agent B | done | 2026-03-03 00:00 | `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | PHYS-SAFE-001B | 경계/클램프/속도캡/NaN 가드 reason code 계측(`reasonCounts`) 추가 완료 |
| PHYS-SAFE-001B | Agent C | done | 2026-03-03 15:35 | `node --experimental-strip-types scripts/qa/strong-shot-burst.ts` 통과(maxTicksToSettle=228), `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | PHYS-SAFE-001C | 쿠션/감쇠 2차 튜닝(`e_bc=0.72`, `linearDamping=0.975`, `cushionPostCollisionSpeedScale=0.95`)로 수렴 시간 단축 및 fallback 0 유지 확인 완료 |
| PHYS-SAFE-001C | Agent C | done | 2026-03-03 15:48 | `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(31 pass), `node --experimental-strip-types scripts/qa/strong-shot-burst.ts` 통과 | - | PHYS-GEO-001A | fallback feature flag 파서(`resolveRecoveryFallbackEnabled`) 보강 및 비활성 시 리스크 경고(`[shot-fallback-risk]`) 로그 분리 완료 |
| PHYS-GEO-001A | Agent C | done | 2026-03-03 16:02 | `node --experimental-strip-types scripts/qa/physics-geometry-sync.ts` 통과, `cd apps/web && npx tsc --noEmit` 통과, `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(31 pass) | - | PHYS-GEO-001B | `table-geometry`에 쿠션 두께/유효 충돌면 오프셋을 공통 상수화하고 웹/물리 경계 참조를 동일 기준으로 정렬 완료 |
| PHYS-GEO-001B | Agent C | done | 2026-03-03 16:10 | `node --experimental-strip-types scripts/qa/physics-render-boundary-contract.ts` 통과, `cd apps/web && npx tsc --noEmit` 통과 | - | PHYS-GEO-001C | `BilliardTable` 쿠션 중심 배치를 물리 경계와 동일한 바깥 배치(`+/- CUSHION_THICKNESS/2`)로 정렬 완료 |
| PHYS-GEO-001C | Agent C | done | 2026-03-03 16:10 | `node --experimental-strip-types scripts/qa/physics-geometry-sync.ts` 통과, `node --experimental-strip-types scripts/qa/physics-render-boundary-contract.ts` 통과 | - | PHYS-ENERGY-001A | 공통 기하 + 렌더 경계 계약 QA 스크립트로 정합성 자동 검증 고정 완료 |
| PHYS-ENERGY-001A | Agent B | done | 2026-03-03 00:00 | `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | PHYS-ENERGY-001B | 물리 스텝 에너지 계측(`kineticEnergyStartJ`, `kineticEnergyEndJ`, `maxPositiveEnergyDeltaJ`) 및 샷 진단 로그 반영 완료 |
| PHYS-ENERGY-001B | Agent B | done | 2026-03-03 00:00 | `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | PHYS-ENERGY-001C | 서브스텝 에너지 캡(`maxSubstepEnergyGainJ`) 기반 속도 재스케일 보호 규칙 반영 완료 |
| PHYS-ENERGY-001C | Agent B | done | 2026-03-03 00:00 | `node --experimental-strip-types --test packages/physics-core/src/room-physics-step.test.ts` 통과(2 pass), `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | AIM-CTRL-001A | 반복 쿠션 충돌 발산 방지 회귀 테스트 추가 완료 |
| AIM-CTRL-001A | Agent A | done | 2026-03-03 00:00 | `cd apps/web && npx tsc --noEmit` 통과, `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | AIM-CTRL-001B | `shared-types` aim-control 계약 추가 및 웹 스토어/카메라 동기화 로직에 모드 게이트 반영 완료 |
| AIM-CTRL-001B | Agent A | done | 2026-03-03 00:00 | `cd apps/web && npx tsc --noEmit` 통과, `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | AIM-CTRL-001C | `KeyM` 토글 기반 `AUTO_SYNC`/`MANUAL_AIM` 전환 및 수동 모드 화살표 조준 연동 완료 |
| AIM-CTRL-001C | Agent A | done | 2026-03-03 00:00 | `cd apps/web && npx tsc --noEmit` 통과, `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | UX-GUIDE-001A | Aim mode UI 토글 버튼 및 세션 저장(`sessionStorage`) 반영 완료 |
| UX-GUIDE-001A | Agent A | done | 2026-03-03 00:00 | `cd apps/web && npx tsc --noEmit` 통과, `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | UX-GUIDE-001B | 하단 조작 가이드 문구를 조작 순서 중심으로 간결화 완료 |
| UX-GUIDE-001B | Agent A | done | 2026-03-03 00:00 | `cd apps/web && npx tsc --noEmit` 통과, `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | UX-GUIDE-001C | 에러/제약 문구를 코드형(`SHOT_LOCKED_*`, `WARN_MISCUE_RISK`, LIMIT_*)으로 일관화 완료 |
| UX-GUIDE-001C | Agent A | done | 2026-03-03 00:00 | `cd apps/web && npx tsc --noEmit` 통과, `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | QA-PLAY-001A | 하단 가이드 레이아웃을 `flex-wrap`/`maxWidth` 기반으로 조정해 모바일 가독성 회귀 방지 완료 |
| QA-PLAY-001A | Agent B | done | 2026-03-03 00:00 | `node --experimental-strip-types scripts/qa/strong-shot-burst.ts` 통과, `node --experimental-strip-types --test apps/game-server/src/lobby/http.test.ts` 통과(30 pass) | - | QA-PLAY-001B | 강샷 연속 시 최대 속도 상한/정지 수렴 검증 QA 스크립트 추가 완료 |
| QA-PLAY-001B | Agent B | done | 2026-03-03 00:00 | `node --experimental-strip-types scripts/qa/korean-key-input-contract.ts` 통과 | - | QA-PLAY-001C | 한글/영문 입력 매핑 계약 회귀 QA 스크립트 추가 완료 |
| QA-PLAY-001C | Agent B | done | 2026-03-03 00:00 | `node --experimental-strip-types scripts/qa/camera-shot-direction-sync-contract.ts` 통과, `cd apps/web && npx tsc --noEmit` 통과 | - | - | 카메라 회전 직후 샷 방향 동기화 계약 QA 스크립트 추가 완료 |
| FAH-API-004A | Agent A | done | 2026-03-09 18:20 | `npm run qa:fah-live-run` 통과, `npm --prefix apps/web run build` 통과 | `34883c3, 9ff0c87` | FAH-API-004B | FAH QA 실행 경로를 physics-core 진단으로 단일화하고 game-server FAH API 경로 혼선 제거 |
| FAH-API-004B | Agent A | done | 2026-03-10 09:30 | `npm --prefix apps/web run build` 통과, `npm run qa:fah-live-run` 통과, `FAH_SEARCH_COUNT=12000 node --experimental-strip-types scripts/qa/fah-multicushion-opt.ts` 통과 | `7625165` | FAH-API-004C | FAH 절대좌표계(장쿠션 0~110, 단쿠션 0~40) 고정 반영 및 6포인트 앵커 최적화 재실행 완료 |
| FAH-API-004D | Agent A | blocked | 2026-03-10 18:55 | `npm --prefix apps/web run build` 실패(`StepRoomPhysicsConfig` 타입 미정합: `linearDampingPerTick`, `cushionPostCollisionSpeedScale`, `clothLinearSpinCouplingPerSec` 등 누락) | - | PHYS-CONFIG-TYPE-001 | 게임모드에서 FAH/테스트 UI 노출 제거 반영 완료, 전체 빌드는 선행 타입 정합 이슈로 차단 |
| FAH-API-004E | Agent A | done | 2026-03-10 11:02 | `rg -n \"테스트 모드\\(FAH\\)|1 하프|3 플러스|하프 시스템\" apps/web/src/components/GameUI.tsx` 결과 없음, `rg -n \"게임 모드|수구궤적\" apps/web/src/components/GameUI.tsx` 통과 | - | FAH-API-004F | 좌측 패널에서 테스트/시스템 전환 버튼 및 시스템 텍스트 제거 완료 |
| FAH-API-004F | Agent A | done | 2026-03-10 11:10 | `rg -n \"path=\\\"/\\\"|path=\\\"/fah\\\"\" apps/web/src/App.tsx` 통과, `rg -n \"GameUI\\s*\\(|showFahPanel\\s*=|mode\\s*=\\s*'game'|mode\\s*=\\s*'fah'\" apps/web/src/components/GameUI.tsx apps/web/src/components/GameScene.tsx` 통과, `cd apps/web && npx tsc --noEmit` 실패(기존 `StepRoomPhysicsConfig` 타입 미정합) | - | PHYS-CONFIG-TYPE-001 | `/` 게임모드와 `/fah` FAH 테스트모드 경로 분리 및 경로 기반 모드 고정 완료 |
| FAH-API-004G | Agent A | done | 2026-03-10 11:22 | `rg -n \"SCORE \\(Target|TURN:\" apps/web/src/components/GameUI.tsx`에서 `/fah` 조건 분기 확인, `rg -n \"FAH Anchors|A0|A45|predict|simulate|calibrate\" apps/web/src/components/GameUI.tsx`로 앵커 패널 단순화 확인, `cd apps/web && npx tsc --noEmit` 실패(기존 `StepRoomPhysicsConfig` 타입 미정합) | - | PHYS-CONFIG-TYPE-001 | `/fah` 좌측 TURN/SCORE 제거, 우측 패널을 앵커 버튼 전용으로 축소, 앵커 샷 시 자동 보정 오프셋 개입 제거 |
| FAH-API-004H | Agent A | done | 2026-03-10 11:35 | `rg -n \"function GameWorld\\(|<GameWorld mode=\\{mode\\}|const isFahMode = mode === 'fah'\" apps/web/src/components/GameScene.tsx` 통과, `rg -n \"gameStore\\.playMode\" apps/web/src/components/GameScene.tsx` 결과 없음, `cd apps/web && npx tsc --noEmit` 실패(기존 `StepRoomPhysicsConfig` 타입 미정합) | - | PHYS-CONFIG-TYPE-001 | GameScene 내부 FAH 분기를 store playMode가 아닌 route mode(`/fah`) 기준으로 고정해 게임모드와 FAH 계산 경로를 분리 완료 |
| FAH-API-004I | Agent A | done | 2026-03-10 11:52 | `rg -n \"path=\\\"/\\\"|path=\\\"/fah\\\"|FahScene|GameScene\" apps/web/src/App.tsx` 통과, `rg -n \"isFahMode = false|isFahMode = true|export function GameScene|export function FahScene\" apps/web/src/components/GameScene.tsx apps/web/src/components/FahScene.tsx` 통과, `cd apps/web && npx tsc --noEmit` 실패(기존 `StepRoomPhysicsConfig` 타입 미정합) | - | PHYS-CONFIG-TYPE-001 | GameScene/FahScene 파일 분리 및 라우트별 고정 씬 연결로 mode prop 의존 제거 |
| PHYS-CONFIG-TYPE-001 | Agent A | done | 2026-03-10 11:59 | `npm --prefix apps/web run build` 통과, `rg -n "predict|simulate|calibrate|FAH_HISTORY|FAH_PHYSICS_TUNING" apps/web/src/components/FahUI.tsx` 결과 없음 | - | FAH-API-004J | `StepRoomPhysicsConfig` 타입 누락 필드 정합 복구 후 `/fah` UI를 앵커 버튼 전용으로 단순화(예측/시뮬/로그 패널 제거) 완료 |
| FAH-API-004J | Agent A | done | 2026-03-10 13:00 | `npm --prefix apps/web run build` 통과 | - | FAH-API-004K | 게임 모드에서 누락된 당점/수구-목적구 겹침 오버레이 UI를 GameUI에 최소 복구 완료 |
| FAH-API-004K | Agent A | done | 2026-03-10 13:02 | `npm --prefix apps/web run build` 통과 | - | FAH-API-004L | 게임 모드 하단 패널에 당점 원형 UI(마커/9분할 가이드) 복구 완료 |
