...
### Phase S. 쿠션 접촉시간 근사 물리 통합 (2026-02-24)

#### PHYS-CT-001 근사 모델 규칙 확정
- `PHYS-CT-001`: 직각 입사 + 사이드스핀 조건에서 `tan(theta)` 접촉시간 근사식(속도 의존 `v^-6/5`)과 기준 상수(`mu_bc`, `alpha`, `v_ref`)를 물리 명세에 확정

#### PHYS-CT-002 물리 문서 동기화
- `PHYS-CT-002`: `docs/Physics-Spec.md`, `docs/Physics-Spec_kr.md`에 쿠션 접촉시간 근사 항목/상수/런타임 처리 순서 동시 반영

#### PHYS-CT-003 런타임 충돌 로직 반영
- `PHYS-CT-003`: game-server 쿠션 충돌 처리에 접촉시간 근사 throw 계산 모듈 추가 및 `mu_bc=0.14` 반영

#### PHYS-CT-004 단위 테스트 추가
- `PHYS-CT-004`: 접촉시간 근사 로직 단위테스트(무스핀 0도, 저속일수록 각 증가, 스핀 방향성, 최대각 상한) 추가

#### PHYS-CT-005 QA 시뮬레이션 표 생성
- `PHYS-CT-005`: 10%~100%(10% 단위) 반사각 표를 생성하는 QA 스크립트 추가 및 단조성 검증 자동화

#### PHYS-CT-006 스펙 가드 검증
- `PHYS-CT-006`: 변경 파일 대상 `spec_guard` 통과로 문서 동기화/규칙 위반 여부 확인

#### PHYS-CT-007 통합 회귀 검증
- `PHYS-CT-007`: `lobby/http.test.ts` 포함 회귀 테스트 통과 후 상태 문서/커밋 기준 정리

### Phase T. QA 실행 관측성 보강 (2026-02-24)

#### QA-OBS-001 QA 로그/실행환경 하드닝
- `QA-OBS-001A`: 루트 `package.json`에 `"type": "module"`을 명시해 루트 QA 스크립트 실행 시 `MODULE_TYPELESS_PACKAGE_JSON` 경고 제거
- `QA-OBS-001B`: `scripts/qa/collect-play-errors.ts`에 `run_started`/`run_completed` JSON 로그를 추가해 soak 배치 완료 여부를 로그 파일만으로 추적 가능하게 보강

#### QA-OBS-002 soak 진행 추적 강화
- `QA-OBS-002A`: `collect-play-errors.ts` heartbeat 로그 주기를 환경변수로 제어 가능하게 확장하고 장시간 배치에서 중간 진행률 모니터링 가능하도록 보강

#### QA-OBS-003 팀 루프 로그 요약 표준화
- `QA-OBS-003A`: `scripts/agents/v2_team_loop.sh` 종료 시 pass/fail 및 로그 경로를 JSON 한 줄로 출력해 CI 파서가 성공 여부를 안정적으로 수집할 수 있게 보강

#### QA-OBS-004 테스트 에이전트 로그 요약 표준화
- `QA-OBS-004A`: `scripts/agents/v2_test_agent.sh` 종료 시 회귀/soak 로그 경로와 soak `errorCount`를 JSON 한 줄로 출력해 팀 루프 실패 분석 자동화를 보강

#### QA-OBS-005 팀 루프 JSON 파서 스모크
- `QA-OBS-005A`: `scripts/qa/parse-team-loop-summary.ts` 스크립트를 추가해 팀 루프 출력의 마지막 JSON을 파싱하고 `status=pass`/필수 필드 존재를 자동 검증

#### QA-OBS-006 테스트 에이전트 JSON 파서 스모크
- `QA-OBS-006A`: `scripts/qa/parse-test-agent-summary.ts` 스크립트를 추가해 test-agent 출력의 마지막 JSON을 파싱하고 `status=pass`/`soakErrorCount=0`/필수 필드 존재를 자동 검증

#### QA-OBS-007 무인 QA 자동 루프 스크립트
- `QA-OBS-007A`: `scripts/agents/v2_auto_loop.sh`를 추가해 팀 루프 실행 결과를 JSON 파서로 검증하며 N회 반복 자동 실행(실패 시 즉시 중단)하도록 보강

#### QA-OBS-008 자동 루프 실패 요약 표준화
- `QA-OBS-008A`: `scripts/agents/v2_auto_loop.sh` 실패 경로에서 `cycle`, `totalCycles`, `reason`을 포함한 JSON 한 줄을 출력해 무인 실행 실패 분석 자동화를 보강

#### QA-OBS-009 자동 루프 완료 요약 파서 스모크
- `QA-OBS-009A`: `scripts/qa/parse-auto-loop-summary.ts`를 추가해 자동 루프 출력의 마지막 JSON(`status=completed`)을 파싱/검증하고 완료 사이클 수를 자동 확인

#### QA-OBS-010 운영 문서 실행 가이드 갱신
- `QA-OBS-010A`: `docs/Unity-Team-Loop_kr.md`에 `v2_auto_loop.sh`/`qa:v2-auto-loop-summary` 사용법과 JSON 로그 해석 방법을 추가해 운영자 재실행 절차를 표준화

### Phase U. Unity 런타임 연동 부트스트랩 (2026-02-24)

#### QA-OBS-011 Unity 플레이 런타임 진입선 구축
- `QA-OBS-011A`: `BhcRoomApiDriver`를 추가해 Unity에서 guest 로그인→room 생성/입장/시작→룸 폴링→샷 제출까지 최소 플레이 루프를 자동 수행하도록 보강하고 `v2_dev_agent`/README를 동기화

#### QA-OBS-012 Unity 룸 동기화 고도화
- `QA-OBS-012A`: Unity 런타임을 룸 detail 폴링에서 SSE(`/lobby/rooms/:id/stream`) 구독 방식으로 확장해 turn/shot 이벤트 지연을 줄이고 폴링 fallback을 유지

#### QA-OBS-013 Unity 수동 플레이 검증 체크리스트
- `QA-OBS-013A`: `unity-client/README.md`에 실제 플레이 테스트 순서(서버 실행, Scene 세팅, 샷 입력, 기대 로그/실패 신호)를 체크리스트로 추가해 사용자가 바로 수동 검증 가능하도록 보강
...
### Phase V. 코어 물리/조작 안정화 (배포 전 선행, 2026-03-03)

#### SYS-CORE-001 물리 런타임 구조 정리
- `SYS-CORE-001A`: `apps/game-server/src/lobby/http.ts`의 `stepRoomPhysics`/충돌 루프를 독립 모듈(`packages/physics-core`)로 추출 가능한 형태로 분리(동작 동일 유지)
- `SYS-CORE-001B`: 고정 timestep/서브스텝 설정을 상수 모듈로 분리하고 룸별 설정 스냅샷 로그를 추가
- `SYS-CORE-001C`: 샷 단위 진단 로그(`shotId`, tick 수, 종료 사유, 최대속도, NaN 가드 발동 여부)를 표준화

#### PHYS-SAFE-001 공 이탈 방지 로직 정교화
- `PHYS-SAFE-001A`: 공 이탈/경계 클램프/강제 복귀 발생 지점을 reason 코드와 함께 계측(teleport 의존도 측정)
- `PHYS-SAFE-001B`: 쿠션/감쇠 파라미터 1차 튜닝으로 일반 샷에서 강제 복귀 빈도 감소(강샷 제외 기준 포함)
- `PHYS-SAFE-001C`: 강제 복귀 fallback을 feature flag로 분리하고, 활성 시 경고 이벤트를 QA 로그에 남기도록 정리

#### PHYS-GEO-001 쿠션 높이/치수 정합성 통합
- `PHYS-GEO-001A`: 쿠션 높이, 공 반지름, 유효 충돌면 오프셋을 공통 상수 파일로 정의(렌더/물리 공용)
- `PHYS-GEO-001B`: 웹 렌더 레이어와 물리 충돌 경계가 동일 상수를 참조하도록 교체
- `PHYS-GEO-001C`: 상수 불일치 시 실패하는 정합성 QA 스크립트 추가(렌더 경계 vs 물리 경계)

#### PHYS-ENERGY-001 충돌 후 속도/회전 재증가 문제 해결
- `PHYS-ENERGY-001A`: 공-공/공-쿠션 충돌 전후 운동에너지 로그를 추가하고 케이스별 증감률 리포트 생성
- `PHYS-ENERGY-001B`: 반발계수/마찰/회전 전달식을 재검토하여 충돌 후 비정상 에너지 증가를 억제하는 보호 규칙 적용
- `PHYS-ENERGY-001C`: 연속 충돌 회귀 테스트(쿠션 반복, 다중 공 충돌)에서 속도/회전 발산이 없는지 검증

#### AIM-CTRL-001 카메라-큐 동기화 모드 옵션화
- `AIM-CTRL-001A`: 조작 모드를 `AUTO_SYNC`/`MANUAL_AIM`으로 분리하는 상태 모델과 입력 계약 정의
- `AIM-CTRL-001B`: 수동 화살표 조준 모드 구현(카메라 회전과 큐 방향 decouple)
- `AIM-CTRL-001C`: UI 토글/기본값/세션 저장 동작을 추가하고 모드 전환 회귀 테스트

#### UX-GUIDE-001 입력 UX 문구 정리
- `UX-GUIDE-001A`: 룸 하단 조작 가이드 문구를 실제 입력 순서(마우스 이동/드래그, WASD, 모드 토글) 기준으로 재작성
- `UX-GUIDE-001B`: 에러/제약 문구를 짧고 일관된 형식으로 정리(샷 잠금, 턴 아님, 입력 범위 초과)
- `UX-GUIDE-001C`: UI 스모크 점검(데스크톱/모바일 폭)으로 줄바꿈/가독성 회귀 확인

#### QA-PLAY-001 플레이 회귀 케이스 고정
- `QA-PLAY-001A`: "강샷 연속" 시나리오 QA 스크립트 추가(속도/스핀 상한, 종료 수렴 시간 검증)
- `QA-PLAY-001B`: "한글키 입력" 환경에서 조작키 충돌/미입력 여부 확인 스크립트 추가
- `QA-PLAY-001C`: "카메라 회전 직후 샷 방향" 일치성 검증 스크립트 추가
