...
### Phase X. /test Sandbox 모드 추가 (2026-03-03)

#### SBX-001 샌드박스 작업 등록
- `SBX-001`: `/test` 샌드박스 개발 작업을 마이크로 태스크(001~009)로 등록하고 순차 수행 계획을 고정

#### SBX-002 샌드박스 진입 라우트 연결
- `SBX-002`: `/test/sandbox` 라우트를 추가하고 `/test` 목록에 Sandbox 카드 진입점을 노출

#### SBX-003 샌드박스 입력 모델/기본값 추가
- `SBX-003`: 샌드박스 전용 입력 타입(공 초기 위치, 샷 파라미터)과 기본 프리셋 데이터를 정의

#### SBX-004 샌드박스 제어 패널 UI 구현
- `SBX-004`: 공 초기 위치/샷 파라미터를 슬라이더+숫자 입력으로 편집하는 컨트롤 패널을 구현

#### SBX-005 샌드박스 실행/리셋 연동
- `SBX-005`: `simulateShot` 실행, 결과 렌더, Reset 플로우를 샌드박스 페이지에 연동

#### SBX-006 샌드박스 프리셋 적용 기능
- `SBX-006`: 프리셋 선택 시 입력값을 즉시 치환하고 재실행 가능한 상태로 반영

#### SBX-007 샌드박스 단축키 지원
- `SBX-007`: `R` 실행, `C` 리셋, `1~5` 프리셋 단축키를 추가하고 입력 포커스 충돌을 방지

#### SBX-008 샌드박스 JSON 내보내기
- `SBX-008`: 공 초기 위치+샷 파라미터만 포함한 JSON 내보내기를 구현하고 물리 상수 편집 경로를 차단

#### SBX-009 샌드박스 검증 및 실행 확인
- `SBX-009`: web 빌드/테스트 스모크 및 로컬 dev 서버 재시작으로 `/test/sandbox` 동작을 검증

#### SBX-012 프레임별 속도 패널 추가
- `SBX-012`: `/test` 시뮬레이션 화면들에 프레임별 선속도/각속도 패널을 추가하고 프레임 클릭 이동을 지원

#### SBX-013 수구 프레임별 회전방향 패널 추가
- `SBX-013`: 공별 속도 영역 하단에 수구 프레임별 회전방향(X/Y/Z)과 축별 부호를 표시

#### SBX-014 현재 프레임 회전방향 카드 표시
- `SBX-014`: 현재 프레임 속도 카드(수구/제1적구/제2적구)에서 축별 회전방향(X/Y/Z)을 즉시 확인 가능하도록 표시

#### SBX-015 현재 프레임 회전량 수치 표시
- `SBX-015`: 현재 프레임 속도 카드에서 회전방향을 부호만이 아닌 축별 회전량 수치(`spinX/Y/Z`)로 표시

#### SBX-016 테스트 장면 축/회전축 오버레이 추가
- `SBX-016`: 테스트 장면 좌하단에 테이블 축(+X/+Y) 안내, 우하단에 회전축(X/Y/Z) 방향 안내 오버레이를 추가
...
#### SBX-021 ura_backcut 1구간 입사각 55도 보정
- `SBX-021`: `/test/ura_backcut` 제1적구 x좌표를 재조정해 첫 쿠션 입사각(법선 기준)을 55도 근처로 보정

#### SBX-022 ura_backcut 2적구 좌측상단 재배치
- `SBX-022`: `/test/ura_backcut` 2적구를 좌측상단으로 이동시켜 구간6 시작점에서 수구-2적구 충돌이 나타나도록 배치
