...
test('다중 공 연속 충돌 + 쿠션 반복에서도 속도/회전이 유한하고 발산하지 않는다', () => {
  ...
  assert.equal(observedPeakSpin <= cfg.cushionMaxSpinMagnitude * 1.01, true);
...
test('저속 백스핀 충돌: 목적구 전달량은 커지고 수구 잔속은 줄어든다', () => {
  const baseConfig = createRoomPhysicsStepConfig();
  const cfg = {
    ...baseConfig,
    clothLinearSpinCouplingPerSec: 0,
    clothAngularSpinCouplingPerSec: 0,
    linearDampingPerTick: 1,
    spinDampingPerTick: 1,
  };
  ...
  assert.equal(backObj.vx > noSpinObj.vx, true);
...
test('백스핀 무충돌 경로: 목적구/쿠션 접촉 전 역방향 속도가 생성되지 않는다', () => {
  ...
  for (let i = 0; i < 700; i += 1) {
    ...
    const forward = cue.vx;
    minForward = Math.min(minForward, forward);
    if (collidedObj || cushionHit) {
      break;
    }
  ...
  assert.equal(cushionHit, false);
  assert.equal(minForward >= -1e-6, true);
});
