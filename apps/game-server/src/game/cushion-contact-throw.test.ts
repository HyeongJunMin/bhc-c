...
test('같은 스핀에서 속도가 높을수록 반사각이 커진다', () => {
  ...
  assert.equal(fast.throwAngleDeg > slow.throwAngleDeg, true);
});
...
test('탑/백스핀은 정면 쿠션 반발 세기에 영향을 준다', () => {
  const topSpin = applyCushionContactThrow({
    ...base,
    axis: 'y',
    vx: 0,
    vy: -5,
  ...
  const backSpin = applyCushionContactThrow({
    ...base,
    axis: 'y',
    vx: 0,
    vy: -5,
  ...
  assert.equal(Math.abs(topSpin.vy) > Math.abs(backSpin.vy), true);
});
...
test('고정 회전(0.8R) + 직각 입사에서 스트로크 증가 시 반사각은 저/중속 구간에서 증가한다', () => {
  ...
  for (let index = 1; index < 7; index += 1) {
    assert.equal(angles[index] >= angles[index - 1], true);
  }
  assert.equal(angles[9] >= angles[7] * 0.85, true);
});
