...
test('대기중 우선 -> 인원 많은 순 -> 최신 생성 순으로 정렬된다', () => {
  const rooms = [
  ...
  assert.equal(sorted[0].playerCount, 2);

  assert.equal(sorted[1].state, 'WAITING');
  assert.equal(sorted[1].playerCount, 1);
  assert.equal(sorted[1].createdAt, '2026-02-22T12:00:00.000Z');

  assert.equal(sorted[2].state, 'WAITING');
  assert.equal(sorted[2].playerCount, 1);
