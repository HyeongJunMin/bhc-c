...
export function deriveFahDynamicPhysicsProfile(
  ...
  const restitution = clamp(base.cushionRestitution * (1 - 0.06 * blend), 0.68, 0.82);
  const contactFriction = clamp(base.cushionContactFriction * (1 + 0.28 * blend), 0.08, 0.22);
  const postSpeedScale = clamp(base.cushionPostCollisionSpeedScale * (1 - 0.02 * blend), 0.9, 0.99);
  const clothSpinCoupling = clamp(base.clothLinearSpinCouplingPerSec * (1 + 0.35 * blend), 1.8, 6.0);
  const spinDamping = clamp(base.spinDampingPerTick * (1 - 0.008 * blend), 0.975, 0.997);
  const linearDamping = clamp(base.linearDampingPerTick * (1 + 0.002 * (1 - blend)), 0.975, 0.995);
